	...
	select mlvt.lecture_vt_No
		...
		from 
			(select row_number() over(partition by lectures[1][1] order by mlvt.matchedat asc) as rn, matchedat
				, mlvt.lectures[1][1] as lecture_vt_No, mlvt.lectures[1][2][2] as student_name
				, mlvt.matchedteacher[1] as teacher_user_No, mlvt.matchedteacher[2] as teacher_name
