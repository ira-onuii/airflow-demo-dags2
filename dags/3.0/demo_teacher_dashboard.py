...
def matching_suggestion_lst():
    ...
    for i in range(1000):
        type = ['accepted','refused','wating','auto_refused']
        random_id3.append([random.randrange(3000000,3111000),random.randrange(2000000,2001000),random.randrange(1000000,1001000),random.choice(type), (start_date + timedelta(days=random.randint(0, (end_date - start_date).days))),(start_date + timedelta(days=random.randint(0, (end_date - start_date).days)))])
        
    data = pd.DataFrame(random_id3, columns=['suggestion_id','teacher_id','lecture_id','tag','created_at','updated_at'])

    matching_suggestion_list = data[['suggestion_id','teacher_id','lecture_id','tag','created_at','updated_at']]
    
    matching_suggestion_list.to_sql(
        name= 'matching_suggestion_demo',  # 삽입할 테이블 이름
        con=pg_engine,  # PostgreSQL 연결 엔진
        schema=pg_schema,
        if_exists='replace',  # 테이블이 있으면 삭제 후 재생성
        index=False  # DataFrame 인덱스는 삽입하지 않음
...
def first_round_lst():
    ...
    data = pd.DataFrame(random_id3, columns=['first_round_id','teacher_id','lecture_id','first_round_status','matching_status','schedule_change_whether','created_at','updated_at'])

    matching_suggestion_list = data[['first_round_id','teacher_id','lecture_id','first_round_status','matching_status','schedule_change_whether','created_at','updated_at']]
    
    matching_suggestion_list.to_sql(
        name= 'first_lecture_demo',  # 삽입할 테이블 이름
        con=pg_engine,  # PostgreSQL 연결 엔진
        schema=pg_schema,
        if_exists='replace',  # 테이블이 있으면 삭제 후 재생성
        index=False  # DataFrame 인덱스는 삽입하지 않음
...
def round_lst():
    ...
    data = pd.DataFrame(random_id3, columns=['round_id','teacher_id','lecture_id','round_status','round_datetime','schedule_change_whether','created_at','updated_at'])

    matching_suggestion_list = data[['round_id','teacher_id','lecture_id','round_status','round_datetime','schedule_change_whether','created_at','updated_at']]
    
    matching_suggestion_list.to_sql(
        name= 'round_demo',  # 삽입할 테이블 이름
        con=pg_engine,  # PostgreSQL 연결 엔진
        schema=pg_schema,
        if_exists='replace',  # 테이블이 있으면 삭제 후 재생성
        index=False  # DataFrame 인덱스는 삽입하지 않음
...
matching_suggestion = PythonOperator(
    task_id='matching_suggestion',
    python_callable=matching_suggestion_lst,
    #op_kwargs={"indicator_table": "add_subject_payment","column_name1": "payment_id","column_name2": "amount"},
    provide_context=True,
    dag=dag
)

first_round = PythonOperator(
    task_id='first_round',
    python_callable=first_round_lst,
    #op_kwargs={"indicator_table": "add_subject_payment","column_name1": "payment_id","column_name2": "amount"},
    provide_context=True,
    dag=dag
)

round = PythonOperator(
    task_id='round',
    python_callable=round_lst,
    #op_kwargs={"indicator_table": "add_subject_payment","column_name1": "payment_id","column_name2": "amount"},
    provide_context=True,
    dag=dag
)
