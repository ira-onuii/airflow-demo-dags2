...
def save_results_to_s3(**context):
    ...
    save_to_s3_with_hook(df, 'onuii-data-pipeline', 'live',table_name, filename)
...
def incremental_extract():
    ...
    max_updated_data = f'select max(update_datetime) as max_update_datetime from {pg_schema}."{trino_schema}.{table_name}"'
    max_updated_data_result =  pd.read_sql(max_updated_data, pg_engine)
    max_updatedat = max_updated_data_result['max_update_datetime'].iloc[0]
    ...
    today_data = f'''
       select 
        "lecture_vt_no","student_user_no","lecture_subject_id","student_type","tutoring_state","payment_item","next_payment_item","current_schedule_no","stage_max_cycle_count","stage_free_cycle_count","stage_pre_offer_cycle_count","stage_offer_cycle_count","create_datetime","update_datetime","last_done_datetime","application_datetime","memo","total_subject_done_month","reactive_datetime"
...
# 
    ...
    df_union_all['row_number'] = df_union_all.sort_values(by = ['update_datetime'], ascending = False).groupby(['id']).cumcount()+1
